...
class BillServiceTest {
	...
	void createBill_WhenMemberNotFound_ShouldThrowException() {
		...
		assertThatThrownBy(() -> billService.createBill(List.of(10L), 99L)).isInstanceOf(MemberNotFoundException.class);
	}
	...
	void createBill_WhenBookAlreadyIssued_ShouldThrowException() {
		...
		when(memberRepo.findById(1L)).thenReturn(Optional.of(member));
		when(bookRepo.findAllById(List.of(10L))).thenReturn(List.of(book1));
		when(billRepo.save(any(Bill.class))).thenAnswer(i -> i.getArgument(0));

		assertThatThrownBy(() -> billService.createBill(List.of(10L), 1L))
	...
	void updateBill_WhenBillNotFound_ShouldThrowException() {
		...
		assertThatThrownBy(() -> billService.updateBill(99L, new Bill())).isInstanceOf(BillNotFoundException.class);
	}
	...
	void updateBill_WhenMemberNotFound_ShouldThrowException() {
		...
		assertThatThrownBy(() -> billService.updateBill(1L, updated)).isInstanceOf(MemberNotFoundException.class);
	}
}
